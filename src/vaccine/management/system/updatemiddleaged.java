...
public class updatemiddleaged extends JFrame implements ActionListener {
     ...
     updatemiddleaged()
     {
        ...
        JLabel l2 = new JLabel("UPDATE MIDDLE AGED CITIZENS DETAILS");
...
//           
//           
            String qry = "update middle set name='"+name+"',age='"+age+"',gender='"+gender+"',vaccine='"+vaccine+"',status='"+status+"',phone='"+phone+"',email='"+email+"' where aadhar="+t8.getText();
            try{
                Conn conect = new Conn();
             int i =  conect.s.executeUpdate(qry);
             if(i>0)
              JOptionPane.showMessageDialog(null,"Middle aged Record Updated");
            }catch(Exception ee){
...
//                 JOptionPane.showMessageDialog(null,"Invalid Aadhar");
    ...
    {
        new updatemiddleaged().setVisible(true);   //object of the class
    }
}
