...
public class updatechild extends JFrame implements ActionListener {
     ...
     updatechild()
     {
       ...
       aad.setFont(new Font("Tahoma",Font.PLAIN,17));
        ...
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("Vaccine/Management/System/icons/ch.jpg"));
        Image i2 = i1.getImage().getScaledInstance(500,600,Image.SCALE_DEFAULT);
        ImageIcon i3 = new ImageIcon(i2);
        JLabel l1 = new JLabel(i3);
        l1.setBounds(400,100,450,550);
        add(l1);
        
        JLabel l2 = new JLabel("UPDATE CHILD'S DETAILS");
...
//           
         ...
         else if(ae.getSource()==b3)
        {
          ...
          {
              Conn p = new Conn();
              String sql = "select * from childeren where aadhar = ?";
...
//           
//           
            String qry = "update childeren set name='"+name+"',age='"+age+"',gender='"+gender+"',vaccine='"+vaccine+"',status='"+status+"',phone='"+phone+"',email='"+email+"' where aadhar="+t8.getText();
            try{
                Conn conect = new Conn();
             int i =  conect.s.executeUpdate(qry);
             if(i>0)
              JOptionPane.showMessageDialog(null,"Child's Record Updated");
            }catch(Exception ee){
...
//                 JOptionPane.showMessageDialog(null,"Invalid Aadhar");
    ...
    {
        new updatechild().setVisible(true);   //object of the class
    }
}
